...
new_data_file = open('all_data_postprocess.tsv', 'w')
new_data_file.write('user_id\tarticle_id\trevision_id\tnamespace\ttimestamp\n') # header

#   data manipulation 
from datetime import datetime
raw_data_file = open('training.tsv')
...
for line in raw_data_file:
    ...
    dt = (2010-timestamp_dt.year)*12 + 9-timestamp_dt.month +(1-timestamp_dt.day)/30.0
    attr[4] = dt
...
datafile = open('all_data_postprocess.tsv')
datafile.readline()
for line in datafile:
    ...
    if (timestamp > model_tst+5) and (timestamp < model_begin):     #TODO
...
for user in users:
    ...
    user_frstedit[user] = model_tst+5
...
data_file = open('all_data_postprocess.tsv')
...
# edit times
    if (timestamp > model_tst+5) and (timestamp <= model_begin):
        user_solus[user] += 1
...
# recent edit times
    if (timestamp > model_tst+5) and (timestamp <= model_tst+10):
...
new_data = open('all_data_features.tsv', 'w')
